...
session_start();
require "config.php";
require "smtp_mailer_bank.php"; // Include your mailer function
...
if (isset($_POST['action']) && $_POST['action'] === 'send_otp') {
    ...
    // Securely check if email exists in bank_details
    $stmt_check = $con->prepare("SELECT email FROM bank_details WHERE email = ?");
    ...
    if ($result->num_rows === 1) {
        ...
        // Securely store OTP in the new bank table
        $stmt_otp = $con->prepare("
            INSERT INTO otp_verification_bank (email, otp, expires) 
            VALUES (?, ?, ?)
        ...
        // Send the email
        $html_msg = "<h2>SUSTainable Bank Password Reset</h2>
                     <p>Your One-Time Password (OTP) is: <b>$otp</b></p>
                     <p>This code is valid for 10 minutes.</p>";
        
        // This uses your existing smtp_mailer.php file
        if (smtp_mailer($email, "SUSTainable Bank - Your Password Reset OTP", $html_msg)) {
            $success_message = "An OTP has been sent to your email.";
            $show_otp_field = true;
            $_SESSION['bank_otp_email'] = $email; // Use a bank-specific session
        } else {
            $error_message = "Could not send email. Please try again later.";
        }
    } else {
        $error_message = "This email is not registered with a bank account.";
    }
    $stmt_check->close();
}
...
if (isset($_POST['action']) && $_POST['action'] === 'verify_otp') {
    ...
    $email = $_SESSION['bank_otp_email'] ?? null;

    if ($email === null) {
        $error_message = "Your session expired. Please start over.";
    } else {
        // Securely check the OTP from the bank table
        $stmt_verify = $con->prepare("SELECT otp FROM otp_verification_bank WHERE email = ? AND expires > ?");
        $current_time = time();
        ...
        if ($result->num_rows === 1) {
            ...
            if ($otp_entered === $correct_otp) {
                // SUCCESS!
                // Delete the OTP
                $stmt_del = $con->prepare("DELETE FROM otp_verification_bank WHERE email = ?");
                $stmt_del->bind_param("s", $email);
                $stmt_del->execute();
                
                // Set session variable to allow password update
                $_SESSION['bank_can_update_password'] = $email;
                header("Location: update_password_bank.php");
                exit;
            } else {
...
<head>
    ...
    <title>Forgot Password - SUSTainable Bank</title>
    <style>
        :root {
        ...
        .button:hover { box-shadow: 0 5px 15px rgba(26, 115, 232, 0.3); }
        .login-link {
...
<body>
    <div class="form-container">
        ...
        <?php if ($show_otp_field): ?>
            ...
            <p class="subtitle">Enter the 6-digit OTP we sent to <?php echo htmlspecialchars($_SESSION['bank_otp_email']); ?>.</p>
            <form action="forgot_password_bank.php" method="POST">
        ...
        <a href="bank_login.php" class="login-link">&larr; Back to Login</a>
    </div>
</body>
</html>
