...
if(isset($_POST["create_account"])) {
    ...
    $stmt1 = $con->prepare("SELECT email FROM bank_details WHERE email = ?");
    ...
    if($result->num_rows > 0) {
        ...
        $current_balance = 50000; // Default balance

        $stmt2 = $con->prepare("INSERT INTO bank_details (email, name, password, account_number, current_balance) VALUES (?, ?, ?, ?, ?)");
        // We store the $hashed_password, not the $password
        $stmt2->bind_param("ssssi", $email, $name, $hashed_password, $account_number, $current_balance);
...
<head>
    ...
    <title>Create Bank Account - SUSTainable Bank</title>
    <style>
        :root {
...
<body>
    <div class="signup-container">
        ...
        <p class="subtitle">Join SUSTainable Bank Today</p>
        ...
        <form action="bank_sign_up.php" method="POST" class="form-container">
        ...
        <a href="bank_login.php" class="login-link">Already have an account? Login</a>
    </div>
